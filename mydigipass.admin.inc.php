...
function mydigipass_admin_settings($form_state) {
  ...
  // The Logout feature is not yet supported in Drupal 6 because of identified
  // issues with menu caching: if the menu was generated when a user was logged
  // on who was not authenticated by MYDIGIPASS.COM, then no user will have the
  // logout feature, because all authenticated users received the cached menu
  // that doesn't have the logout feature.
  /*
  $form['features'] = array(
    '#type' => 'fieldset',
  ...
  $form['features']['mydigipass_logout_feature'] = array(
    ...
    '#description' => "By enabling the logout feature, users will be prompted whether they also want to log out of MYDIGIPASS.COM when logging out of this site. When logging out, the MYDIGIPASS.COM logout dialog will be shown where users can choose to also logout from MYDIGIPASS.COM or open their Launch Pad.<br />Because of a limitation in Drupal 6, it is only possible to add the logout feature to logout links that are available in Drupal menu's. Logout links generated in blocks, themes, other modules, or other places can not be updated with the logout feature automatically.",
  );
  */

  $form['#submit'][] = 'mydigipass_admin_settings_submit';

  return system_settings_form($form);
}

/**
 * Submit handler connected to the Account Settings form.
 *
 * The button is present on 'admin/settings/mydigipass' and on
 * 'admin/settings/mydigipass/account_settings'.
 *
 * The saving of the different form values is already handled by
 * system_settings_form(). This submit handler will set the mydigipass_origin
 * variable based on the selected environment.
 */
function mydigipass_admin_settings_submit($form, &$form_state) {
  if ($form_state['values']['mydigipass_environment'] == 'test') {
    variable_set('mydigipass_origin', 'https://sandbox.mydigipass.com');
  }
  elseif ($form_state['values']['mydigipass_environment'] == 'production') {
...
function mydigipass_admin_settings_advanced_form($form_state) {
  ...
  $form['endpoint_urls']['mydigipass_origin'] = array(
    '#type' => 'textfield',
    '#title' => t('Data origin'),
    '#default_value' => variable_get('mydigipass_origin', ''),
    '#description' => t('The base URL of the used environment.'),
    '#required' => TRUE,
  );
  ...
  if (variable_get('mydigipass_environment', '') != 'custom') {
    ...
    // In Drupal 6, form fields that are disabled and that are required must
    // also have a value. To avoid validation errors, these form fields are not
    // required when the environment is not set to 'custom'.
    $form['endpoint_urls']['mydigipass_endpoint_url_dp_connect_js']['#required'] = FALSE;
    $form['endpoint_urls']['mydigipass_endpoint_url_token_endpoint']['#required'] = FALSE;
    $form['endpoint_urls']['mydigipass_endpoint_url_data_endpoint']['#required'] = FALSE;
    $form['endpoint_urls']['mydigipass_endpoint_url_api_uuids_connected']['#required'] = FALSE;
    $form['endpoint_urls']['mydigipass_endpoint_url_api_uuids_disconnected']['#required'] = FALSE;
  }
  ...
  $url = variable_get('mydigipass_origin', '');
  if ($url == '') {
...
function mydigipass_admin_settings_advanced_form_validate($form, &$form_state) {
  ...
  if (empty($form_state['values']['mydigipass_validate_certificate_cafile'])) {
    $result_without_validation = _mydigipass_http_request($url);
    $params = array(
    ...
    $result_with_validation = _mydigipass_http_request($url, $params);
    ...
    elseif (($result_without_validation->code > 0) && ($result_with_validation->code <= 0)) {
      form_set_error('mydigipass_validate_certificate_cafile',
        t('The certificate of !url could not be validated. This probably means that the PHP application server has not been configured to have access to valid SSL root certificates. Either reconfigure PHP or enter the path to a Certificate Authority file. Check the module homepage at !url2 for instructions on where you can obtain a Certificate Authority file.', array('!url' => $url, '!url2' => l(t('http://drupal.org/project/mydigipass'), 'http://drupal.org/project/mydigipass'))));
    }
    elseif (($result_without_validation->code > 0) && ($result_with_validation->code > 0)) {
