...
function mydigipass_callback() {
  ...
  if (empty($client_secret) || empty($client_secret)) {
    drupal_set_message(t('The MYDIGIPASS.COM module has not been correctly configured on this Drupal installation. Either the client_id or the client_secret has not been properly configured.'), 'error');
    watchdog('mydigipass', 'The MYDIGIPASS.COM module has not been correctly configured on this Drupal installation. Either the client_id or the client_secret has not been properly configured.', array(), WATCHDOG_ERROR);
    return $return_or_error;
  }

  // Check if the callback contained the error parameter. If so, something must have gone wrong.
  if (isset($_GET['error'])) {
    $error = $_GET['error'];
    $error_description = $_GET['error_description'];

    drupal_set_message(t('An error occured: Error: @error - Error description: @error_description', array('@error' => $error, '@error_description' => $error_description)), 'error');
    watchdog('mydigipass', 'An error occured: Error: @error - Error description: @error_description', array('@error' => $error, '@error_description' => $error_description), WATCHDOG_ERROR);
    return $return_or_error;
  }

  // Extract the authorisation code
  $code = $_GET['code'];
  // Validate using regular expression that the authorisation code exists out of [0-9a-z] characters
  if (preg_match('/^[0-9a-z]+$/', $code) != 1) {
    drupal_set_message(t('An error occured: the authorisation code does not have the expected format'), 'error');
    watchdog('mydigipass', 'An error occured: the authorisation code does not have the expected format', array(), WATCHDOG_ERROR);
    return $return_or_error;
  }

  // Extract the state
  $state = $_GET['state'];
  // Validate using regular expression that, if the state parameter is set, the state exists out of [0-9a-z] characters
  if (! empty($state) && (preg_match('/^[0-9a-z]+$/', $code) != 1)) {
    drupal_set_message(t('An error occured: the state parameter does not have the expected format'), 'error');
    watchdog('mydigipass', 'An error occured: the state parameter does not have the expected format', array(), WATCHDOG_ERROR);
    return $return_or_error;
  }
  ...
  watchdog('mydigipass', 'Connection from MYDIGIPASS.COM user with mail %email and UUID %uuid.', array('%email' => $user_data_array['email'], '%uuid' => $user_data_array['uuid']));
  ...
  if ($result == 1) {
    // The MYDIGIPASS.COM end-user is already linked to an existing Drupal user, let's authenticate the Drupal user
    global $user;
    ...
    if (! $success) {
      // The user didn't pass the user_external_login function, so probably the user
      // account is disabled or locked. Destroy the current session and make sure the
      // user is set to the anonymous user.
      session_destroy();
    ...
    // The MYDIGIPASS.COM end-user is not yet linked to an existing Drupal user.

    // Check if the user is logged in and if the state parameter is set. If so, then the user
    // clicked the 'Link to MYDIGIPASS.COM' button in his profile.
    if (user_is_logged_in() && !empty($_SESSION['mydigipass_link_code']) && ($state == $_SESSION['mydigipass_link_code'])) {
      global $user;
    ...
    // Redirect to the user edit form (which contained the MYDIGIPASS.COM connect button)
...
function _mydigipass_consume_authorisation_code($code) {
  ...
  elseif (function_exists('fsockopen') && in_array('openssl', get_loaded_extensions())) {
    ...
    drupal_set_message(t('This PHP installation lacks the necessary functions to make outbound connections.'));
    watchdog('mydigipass', 'This PHP installation lacks the necessary functions to make outbound connections.', array(), WATCHDOG_ERROR);
    return FALSE;
  }
  ...
  // Validate using regular expression that the access token exists out of [0-9a-z] characters
  if (preg_match('/^[0-9a-z]+$/', $access_token) != 1) {
    drupal_set_message(t('An error occured: the access token does not have the expected format'), 'error');
    watchdog('mydigipass', 'An error occured: the access token does not have the expected format. Returned token was %token', array('%token' => $access_token), WATCHDOG_ERROR);
    return FALSE;
  }
...
function _mydigipass_callback_get_access_token_using_curl($code) {
  ...
  $post_data = 'code=' . $code . '&client_secret=' . variable_get('mydigipass_client_secret', '') . '&client_id=' . variable_get('mydigipass_client_id', '') . '&redirect_uri=' . url('mydigipass/callback', array('absolute' => TRUE)) . '&grant_type=authorization_code';
  ...
  if ($data === FALSE) {
    drupal_set_message(t('An error occured while contacting MYDIGIPASS.COM.'), 'error');
    watchdog('mydigipass', 'An error occured while executing the curl_exec to the token endpoint: the error was %error', array('%error' => curl_error($ch)), WATCHDOG_ERROR);
  }
  elseif ($http_status == 200) {
    ...
    // MYDIGIPASS.COM did not return a status code 200, so something went wrong
    drupal_set_message(t('An error occured while contacting MYDIGIPASS.COM.'), 'error');
    watchdog('mydigipass', 'An error occured while contacting the token endpoint: the HTTP status code was %http_status', array('%http_status' => $http_status), WATCHDOG_ERROR);
  }
...
function _mydigipass_callback_get_user_data_using_curl($access_token) {
  ...
  if ($http_status == 200) {
    ...
    drupal_set_message(t('An error occured while contacting MYDIGIPASS.COM.'), 'error');
    watchdog('mydigipass', 'An error occured while contacting the user data endpoint: the HTTP status code was %http_status', array('%http_status' => $http_status), WATCHDOG_ERROR);
    return FALSE;
  }
}
...
function _mydigipass_callback_get_access_token_using_fsockopen($code) {
  ...
  // CLEANUP NEEDED: do something else instead of "die" in case of an error
  ...
  if ($fp === FALSE) {
    drupal_set_message(t('An error occured while contacting MYDIGIPASS.COM.'), 'error');
    watchdog('mydigipass', 'An error occured while opening a socket to the token endpoint using fsockopen: the error was %error', array('%error' => $err_msg), WATCHDOG_ERROR);
  }
  else {
    // Create the POST request
    $crlf = "\r\n";
    $post_data = 'code=' . $code . '&client_secret=' . variable_get('mydigipass_client_secret', '') . '&client_id=' . variable_get('mydigipass_client_id', '') . '&redirect_uri=' . url('mydigipass/callback', array('absolute' => TRUE)) . '&grant_type=authorization_code';
    $request = 'POST ' . $url['path'] . (isset($url['query']) ? '?' . $url['query'] : '') . ' HTTP/1.0' . $crlf;
    ...
    // CLEANUP NEEDED: catch errors during this process
    fputs($fp, $request);
    $http_response = '';
    while (!feof($fp)) {
    ...
    if ($http_status_array[1] == '200') {
      // The HTTP request resulted in a HTTP 200 OK response.
      // The HTTP response is the full response including the headers, so extract the response body
      ...
      // MYDIGIPASS.COM did not return a status code 200, so something went wrong
      drupal_set_message(t('An error occured while contacting MYDIGIPASS.COM.'), 'error');
      watchdog('mydigipass', 'An error occured while contacting the token endpoint: the HTTP status code was %http_status', array('%http_status' => $http_status_array[1]), WATCHDOG_ERROR);
    }
  }
...
function _mydigipass_callback_get_user_data_using_fsockopen($access_token) {
  ...
  if ($fp === FALSE) {
    drupal_set_message(t('An error occured while contacting MYDIGIPASS.COM.'), 'error');
    watchdog('mydigipass', 'An error occured while opening a socket to the user data endpoint using fsockopen: the error was %error', array('%error' => $err_msg), WATCHDOG_ERROR);
  }
  else {
    // Create the GET request
    $crlf = "\r\n";
    $request = 'GET ' . $url['path'] . (isset($url['query']) ? '?' . $url['query'] : '') . ' HTTP/1.0' . $crlf;
    $request .= 'Host: ' . $url['host'] . $crlf;
    ...
    if ($http_status_array[1] == '200') {
      ...
      // MYDIGIPASS.COM did not return a status code 200, so something went wrong
      drupal_set_message(t('An error occured while contacting MYDIGIPASS.COM.'), 'error');
      watchdog('mydigipass', 'An error occured while contacting the user data endpoint: the HTTP status code was %http_status', array('%http_status' => $http_status_array[1]), WATCHDOG_ERROR);
    }
  }
